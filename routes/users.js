...
router.get('/', function (req, res, next) {
    dbsql.query('SELECT * FROM users', function (err, rows, fields) {
        if (err)
            throw err;
...
router.post('/add', function (req, res) {
    ...
    dbsql.query('INSERT INTO users SET ?', data, function (err, rows, fields) {
        if (err)
            throw err;
...
router.post('/update/:id', function (req, res) {
    ...
    dbsql.query('UPDATE users SET ? WHERE id = ? ', [data, req.params.id], function (err, rows, fields) {
        if (err)
            throw err;
